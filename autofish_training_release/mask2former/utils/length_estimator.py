...
class LengthEstimator():
    @classmethod
    def skeletonize(cls, binary_mask, method):
    ...
    def get_poly_fit(cls, binary_mask, skeleton_method="zhang", degree=3, subsample_skeleton=None, subsample_fit=None, clip=True):
        ...
        skeleton = cls.skeletonize(np.ascontiguousarray(binary_mask), skeleton_method)
        ...
        if min(first_solution[1], second_solution[1]) == first_solution[1]:
    ...
    @classmethod
    def draw_predicted_lengths(cls, img, fits):
        output_image = copy.deepcopy(img)
        for fit in fits:
            for pixel in fit: #draw fit
                cv2.circle(output_image, (int(pixel[0]), int(pixel[1])), 1, (0, 255, 0), -1)
